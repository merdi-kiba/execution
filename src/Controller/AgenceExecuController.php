<?php

namespace App\Controller;

use App\Entity\AgenceExce;
use App\Form\AgenceExecuType;
use App\Repository\AgenceExceRepository;
use Doctrine\ORM\EntityManagerInterface;
use Symfony\Bundle\FrameworkBundle\Controller\AbstractController;
use Symfony\Component\HttpFoundation\Request;
use Symfony\Component\HttpFoundation\Response;
use Symfony\Component\Routing\Annotation\Route;

class AgenceExecuController extends AbstractController
{
    #[Route('/agence/execu', name: 'app_agence_execu',  methods:['GET'])]
    public function index(AgenceExceRepository $agenceExceRepository): Response
    {
        $agence= $agenceExceRepository->findAll();

        return $this->render('agence_execu/index.html.twig', [compact('agence')]);
    }
    #[Route('/agence/{id<\d+>}', name: 'app_agence', methods:['GET'])]

    public function voir_pins(AgenceExceRepository $agenceExce): Response
    {
        return $this->render('agence_execu/voir.html.twig', compact('agenceExce'));
    }

    // pour creer une entitée le lien est important !!!!!!

    #[Route('/new/agence', name: 'app_new_agence', methods: ['GET', 'POST'])]
    public function new (Request $request, EntityManagerInterface $em)
    {
        $agence = new AgenceExce;

        $form = $this->createForm(AgenceExecuType::class, $agence);

        $form->handleRequest($request);

        if ($form->isSubmitted() && $form->isValid()) {

            $agence = $form->getData();
            $em->persist($agence);
            $em->flush();

            $this->addFlash(
                'success',
                'votre entité est bien ajoutée'
            );
            return $this->redirectToRoute('app_agence_execu');
        }
        return $this->render('agence_execu/new.html.twig', ['form' => $form->createView()]);
    }

    // cette route te permet de modifier une entite !!!!

    #[Route('/edit/{id<\d+>}/agence', name: 'app_edit_agence', methods: ['GET', 'POST'])]

    public function edit(Request $request, EntityManagerInterface $em, AgenceExce $agenceExce)
    {
        $form = $this->createForm(AgenceExecuType::class, $agenceExce);
        $form->handleRequest($request);
        if ($form->isSubmitted() && $form->isValid()) {
            $agenceExce = $form->getData();
            $em->persist($agenceExce);
            $em->flush();

            $this->addFlash(
                'success',
                'votre entité est bien modifiée'
            );
            return $this->redirectToRoute('app_agence_execu');
        }
        return $this->render('agence_execu/edit.html.twig', ['form' => $form->createView()]);
    }

    #[Route('/delete/{id<\d+>}/agence', name: 'app_delete_agence', methods: ['GET', 'POST'])]
    public function delet(AgenceExce $agenceExce, EntityManagerInterface $em)
    {
        $em->remove($agenceExce);
        $em->flush();
        $this->addFlash('error', 'supprimer');
        return $this->redirectToRoute('app_agence_execu');
    }
}
