...
class EntrepriseExecuController extends AbstractController
{
    ...
    public function index(EntrepriseExecuRepository $entrepriseExecuRepository): Response
    {

        $entrepise= $entrepriseExecuRepository->findAll();
    ...
    public function voir_pins(EntrepriseExecuRepository $entrepise): Response
    {
        return $this->render('entreprise_execu/voir.html.twig', compact('entreprise'));
    }
    ...
    {
        $form = $this->createForm(Entre ::class, $entrepriseExecu);
        $form->handleRequest($request);
        if ($form->isSubmitted() && $form->isValid()) {
            $entrepriseExecu = $form->getData();
            $em->persist($entrepriseExecu);
